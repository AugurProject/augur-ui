...
export default function (numShares, limitPrice, side, minPrice, maxPrice, type, sharesFilled, tradeTotalCost) {
  ...
  const limit = new BigNumber(calculatedPrice)
  ...
  const potentialEthProfit = side === BUY ?
    new BigNumber(max.minus(limit).abs()).times(calculatedShares) :
    new BigNumber(limit.minus(min).abs()).times(calculatedShares)

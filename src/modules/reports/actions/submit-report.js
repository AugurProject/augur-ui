...
import { CATEGORICAL, SCALAR } from "modules/common-elements/constants";
...
export const submitReport = ({
  ...
  console.log(
    `submit report ${reportedOutcomeId} on market ${market.id} period ${
      universe.currentReportingWindowAddress
    }...`
  );
  ...
  augur.reporting.submitReport({
    ...
    onSuccess: () => {
      ...
      dispatch(
        updateReport(universeId, market.id, {
          ...(reports[universeId] || {})[market.id],
          isSubmitted: true
        })
      );
      callback(null);

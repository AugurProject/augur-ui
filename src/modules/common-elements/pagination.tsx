...
export const createPagesArray = (page: number, totalPages: number) => {
  ...
  for(let b = -8; b < -1; b++) {
    if (PagesArray[page + b] && PagesArray[page + b].page !== 1) SevenBefore.push(PagesArray[page + b]);
  }
  for(let a = 0; a <= 6; a++) {
    if (PagesArray[page + a] && PagesArray[page + a].page !== totalPages) SevenAfter.push(PagesArray[page + a]);
  }
  ...
  if (ArrayToShow[finalLen - 2].page !== totalPages - 1) ArrayToShow[finalLen - 2] = NullPage;
  return ArrayToShow;
}

const renderPageButtons = (pagesArray: Array<PagesArrayObject>, action: Function) => (
  <>
    {pagesArray.map((page: PagesArrayObject, index: number) =>
      <button
        key={`${page.page}-${index}`}
...
export const Pagination = (props: PaginationProps) => {
  ...
  return (
    <div className={Styles.Pagination}>
      <DirectionButton action={() => action(page - 1)} left disabled={page === 1} />
      {renderPageButtons(createPagesArray(50, 500), action)}
      <span>{page} of {totalPages}</span>
      <DirectionButton action={() => action(page + 1)} disabled={page === totalPages} />
    </div>
  );
};

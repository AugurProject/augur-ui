...
class Notifications extends React.Component<NotificationsProps> {
  ...
  render() {
    const notifications = this.props.notifications
      .map(notificaction => {
        let options = {};
        if (notificaction.type === constants.NOTIFICATION_TYPES.resolvedMarketsOpenOrders) {
          options = {
            buttonAction: () => null,
            Template: OpenOrdersResolvedMarketsTemplate
          }
        }
        else if (notificaction.type === constants.NOTIFICATION_TYPES.reportOnMarkets) {
          options = {
            buttonAction: () => null,
            Template: ReportEndingSoonTemplate
          }
        }
        else if (notificaction.type === constants.NOTIFICATION_TYPES.finalizeMarkets) {
          options = {
            buttonAction: () => null,
            Template: FinalizeTemplate
          }
        }
        else if (notificaction.type === constants.NOTIFICATION_TYPES.marketsInDispute) {
          options = {
            buttonAction: () => null,
            Template: DisputeTemplate
          }
        }
        else if (notificaction.type === constants.NOTIFICATION_TYPES.completeSetPositions) {
          options = {
            buttonAction: () => null,
            Template: SellCompleteSetTemplate
          }
        }
        else if (notificaction.type === constants.NOTIFICATION_TYPES.unsignedOrders) {
          options = {
            buttonAction: () => null,
        ...
        else if (notificaction.type === constants.NOTIFICATION_TYPES.claimReportingFees) {
          options = {
            buttonAction: () => null,

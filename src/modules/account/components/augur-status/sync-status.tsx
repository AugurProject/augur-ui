...
const SyncStatus = (props: SyncStatusProps) => {
  const Synced = (
    <div>
      <span />{constants.SYNCED}
    </div>
  );

  const Syncing = (
    <div>
      <span>{SyncingIcon}</span>{constants.SYNCING}
    </div>
  );

  const BlocksBehind = (
    <div>
      <span>{ImmediateImportance}</span>{constants.SYNCING} <span>{constants.MANY_BLOCKS_BEHIND}</span>
    </div>
  );
  ...
  else if (props.syncPercent >= 99.90) {
    data = {
  ...
  return (
    <div className={classNames(Styles.SyncStatus, {

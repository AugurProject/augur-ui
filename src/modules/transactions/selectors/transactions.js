...
export function formatTransaction(transaction) {
  return {
    ...
    ethTokens:
      transaction.etherWithoutGas && formatEther(transaction.etherWithoutGas),
...
export function formatGroupedTransactions(transactions) {
  const formattedTransactions = transactions
    ...
    .sort(
      (a, b) =>
        getValue(b, "timestamp.timestamp") - getValue(a, "timestamp.timestamp")
    );

  const status = formattedTransactions.reduce((p, transaction) => {
    if (p === FAILED || transaction.status === FAILED) return FAILED;
    if (p === INTERRUPTED || transaction.status === INTERRUPTED)
      return INTERRUPTED;
  ...
  const totalShares = formattedTransactions.reduce(
    (p, transaction) => p.plus(createBigNumber(transaction.numShares.value)),
    createBigNumber(0)
  );
  ...
  return {
    status,
    message: `${
      formattedTransactions[0].type === BUY ? "Buy" : "Sell"
    } ${totalShares.toNumber()} shares of ${
      formattedTransactions[0].data.outcomeName
    }`,
    description: formattedTransactions[0].description,
    timestamp:
      formattedTransactions[formattedTransactions.length - 1].timestamp,

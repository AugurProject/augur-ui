  ...
  (loginAccount, orders, makerFee, settlementFee, range, scalarMinValue) => {
    ...
    const userEther =
      loginAccount.ether != null
        ? createBigNumber(loginAccount.ether, 10)
        : ZERO;
    ...
    } = augur.trading.fees.calculateFxpTradingFees(
      createBigNumber(makerFee, 10),
      createBigNumber(settlementFee, 10)
    );
    ...
    for (let i = 0; i < ordersLength; i++) {
      ...
      fullPrecisionPrice =
        scalarMinValue !== null
          ? augur.trading.shrinkScalarPrice(
              scalarMinValue,
      ...
      if (updatedRunningCost.lte(userEther)) {
        ...
        let remainingShares;
        const feePerShare = speedomatic
          .fix(orderCost.fee.abs())
        ...
        if (order.type === "buy") {
          ...
          remainingShares = speedomatic.unfix(
            remainingEther
              .dividedBy(feePerShare.plus(speedomatic.fix(fullPrecisionPrice)))
              .times(TEN_TO_THE_EIGHTEENTH_POWER)
              .floor()
          );
        }

...
export const loadPendingLiquidityOrders = data => ({ type: LOAD_PENDING_LIQUIDITY_ORDERS, data })

export const addMarketLiquidityOrders = data => ({ type: ADD_MARKET_LIQUIDITY_ORDERS, data })

export const clearMarketLiquidityOrders = data => ({ type: CLEAR_ALL_MARKET_ORDERS, data })

export const updateLiquidityOrder = data => ({ type: UPDATE_LIQUIDITY_ORDER, data })

export const removeLiquidityOrder = data => ({ type: REMOVE_LIQUIDITY_ORDER, data })
  ...
  eachOfSeries(Object.keys(orderBook), (outcome, index, seriesCB) => {
    ...
    eachOfLimit(orderBook[outcome], 1, (order, orderId, orderCB) => {
      ...
      augur.api.CreateOrder.publicCreateOrder({
        ...
        onSent: (res) => {
          dispatch(updateLiquidityOrder({ marketId, order, orderId, outcomeId, updates: { onSent: true, orderId: res.callReturn, txhash: res.hash } }))
          orderCB()
        },
        onSuccess: (res) => {
